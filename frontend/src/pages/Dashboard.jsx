import React, { useEffect, useState } from 'react'
import { motion } from 'framer-motion'
import api, { downloadUrl } from '../services/api.js'
import { FiDownload, FiTrendingUp, FiUsers, FiBarChart2 } from 'react-icons/fi'

import StatCard from '../components/common/StatCard.jsx'
import LoadingSpinner from '../components/common/LoadingSpinner.jsx'

export default function Dashboard() {
  const [stats, setStats] = useState(null)
  const [recent, setRecent] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const run = async () => {
      try {
        const [s, r] = await Promise.all([
          api.get('/stats'),
          api.get('/history?limit=5'),
        ])
        setStats(s.data)
        setRecent(r.data.items)
      } catch (err) {
        // If backend doesn't have stats endpoint, show empty
        setStats({ total_bills: 0, total_units: 0, total_amount: 0 })
      } finally {
        setLoading(false)
      }
    }
    run()
  }, [])

  if (loading) return <LoadingSpinner />

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Dashboard</h1>
          <p className="text-sm text-slate-500 dark:text-slate-300">Track your solar bill conversions</p>
        </div>
      </div>

      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard title="Bills Processed" value={stats?.total_bills ?? 0} icon={FiBarChart2} />

        <StatCard title="Total Units" value={stats?.total_units ?? 0} icon={FiTrendingUp} />
        <StatCard
          title="Total Amount"
          value={stats?.total_amount ? `₹${stats.total_amount}` : '₹0'}
          icon={FiUsers}
        />
      </div>

      <div className="mt-6 rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur shadow-sm ring-1 ring-slate-200/60 dark:ring-slate-700/60 overflow-hidden">
        <div className="p-4 border-b border-slate-200/60 dark:border-slate-700/60">
          <div className="font-semibold">Recent 5 Bills</div>
        </div>
        <div className="p-4 overflow-auto">
          <table className="w-full text-sm">
            <thead className="text-left text-slate-500 dark:text-slate-300">
              <tr>
                <th className="py-2">Consumer</th>
                <th className="py-2">Period</th>
                <th className="py-2">Units</th>
                <th className="py-2">Amount</th>
                <th className="py-2">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200/60 dark:divide-slate-700/60">
              {recent.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-6 text-center text-slate-500 dark:text-slate-300">
                    No bills yet.
                  </td>
                </tr>
              ) : (
                recent.map((b) => (
                  <tr key={b.id}>
                    <td className="py-3">{b.extracted_data?.consumer_name || b.original_filename}</td>
                    <td className="py-3">{b.extracted_data?.billing_period?.from} → {b.extracted_data?.billing_period?.to}</td>
                    <td className="py-3">{b.extracted_data?.units_consumed ?? '-'}</td>
                    <td className="py-3">₹{b.extracted_data?.bill_amount ?? '-'}</td>
                    <td className="py-3">
                        <a
                          href={downloadUrl(b.id)}
                          className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-500 text-white hover:bg-emerald-600 transition"
                        >
                          <FiDownload /> Excel
                        </a>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>
    </motion.div>
  )
}

